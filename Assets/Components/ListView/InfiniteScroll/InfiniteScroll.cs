...
public class InfiniteScroll : UIBehaviour {
    ...
    public void UpdateItems() {
        List<RectTransform> l = new List<RectTransform>(itemList);
        for (int i = 0; i < l.Count; i++)
            onUpdateItem.Invoke(currentItemNo + i, l[i].gameObject);
    }
    ...
    public void ResetScrollView() {
        ...
        List<RectTransform> l = new List<RectTransform>(itemList);
        for (int i = 0; i < l.Count; i++)
            l[i].anchoredPosition = direction == Direction.Vertical ? new Vector2(0, -itemScale * i) : new Vector2(itemScale * i, 0);
